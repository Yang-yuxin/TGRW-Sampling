...
const char* file = "data/out.opsahl-ucsocial";
    ...
    std::cout << "Running with Phase =  " << phase << std::endl;
    // std::cout << "Specify valid Phase, taking default value Phase = 1 " << std::endl;
    ...
    for (int tr = 0; tr < trials; tr++){    
        ...
        #pragma omp parallel
        {
            int phased_walks = num_walk/phase;
            for (int k = 0; k < phase; k++){
                #pragma omp for schedule(dynamic, 10)
                for (int k = 0; k < phased_walks; k++){
